...
public class TileHeight extends org.openrdf.query.algebra.evaluation.function.postgis.raster.attribute.TileHeight implements NativeFunction  {

	 // auto-register for SPARQL environment
  static {
      if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_tileHeight.toString())) {
          FunctionRegistry.getInstance().add(new TileHeight());
      }
  }

  @Override
  public String getURI() {
      return FN_POSTGIS.st_tileHeight.stringValue();
  }
  ...
  public String getNative(KiWiDialect dialect, String... args) {
      if (dialect instanceof PostgreSQLDialect) {
          if (args.length == 1) {
              ...
              return String.format("ST_TileHeight(%s)", geom1,longitude,latitude);
          }
      }
      throw new UnsupportedOperationException(FN_POSTGIS.st_tileHeight.toString()+" function not supported by dialect " + dialect);
  }
