...
public class IsPlanar extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.attribute.IsPlanar implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_isPlanar.toString())) {
            FunctionRegistry.getInstance().add(new IsPlanar());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_isPlanar.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_IsPlanar(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_isPlanar.stringValue()+" function not supported by dialect " + dialect);
    }
