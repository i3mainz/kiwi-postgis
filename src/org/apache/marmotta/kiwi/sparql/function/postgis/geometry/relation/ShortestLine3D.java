...
public class ShortestLine3D extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.relation.ShortestLine3D implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_shortestLine3D.toString())) {
            FunctionRegistry.getInstance().add(new ShortestLine3D());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_shortestLine3D.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 2) {
                ...
                return String.format("ST_ShortestLine3D(%s,%s)", geom1,geom2);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_shortestLine3D.toString()+" function not supported by dialect " + dialect);
    }
