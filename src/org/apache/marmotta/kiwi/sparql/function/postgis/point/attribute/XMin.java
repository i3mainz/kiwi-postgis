...
public class XMin implements NativeFunction {
    ...
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_xMin.toString())) {
            FunctionRegistry.getInstance().add(new XMin());
        }
    }
    ...
    public String getURI() {
        return FN_POSTGIS.st_xMin.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_Xmin(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_xMin.toString()+" function not supported by dialect " + dialect);
    }
