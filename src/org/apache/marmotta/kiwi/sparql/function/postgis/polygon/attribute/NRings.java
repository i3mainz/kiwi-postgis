...
public class NRings implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_nRings.toString())) {
            FunctionRegistry.getInstance().add(new NRings());
        }
    }
    ...
    public String getURI() {
        return FN_POSTGIS.st_nRings.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_NRings(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_nRings.toString()+" function not supported by dialect " + dialect);
    }
