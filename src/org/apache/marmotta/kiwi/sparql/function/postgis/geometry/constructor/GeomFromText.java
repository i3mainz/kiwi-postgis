...
import org.apache.marmotta.kiwi.sparql.function.postgis.geometry.exporter.AsBinary;
import org.apache.marmotta.kiwi.vocabulary.FN_POSTGIS;
import org.openrdf.query.algebra.evaluation.function.FunctionRegistry;

public class GeomFromText extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.constructor.GeomFromText implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_geomFromText.toString())) {
            FunctionRegistry.getInstance().add(new GeomFromText());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_geomFromText.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                 ...
                 */
                return String.format("ST_GeomFromText(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_geomFromText.toString()+" function not supported by dialect " + dialect);
    }
