...
public class RastFromHexWKB extends org.openrdf.query.algebra.evaluation.function.postgis.raster.constructor.RastFromHexWKB implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_rastFromHexWKB.toString())) {
            FunctionRegistry.getInstance().add(new RastFromHexWKB());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_rastFromHexWKB.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                 ...
                 */
                return String.format("ST_RastFromHexWKB(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_rastFromHexWKB.toString()+" function not supported by dialect " + dialect);
    }
