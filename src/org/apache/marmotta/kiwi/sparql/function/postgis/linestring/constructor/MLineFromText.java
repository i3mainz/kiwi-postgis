...
public class MLineFromText extends org.openrdf.query.algebra.evaluation.function.postgis.linestring.constructor.MLineFromText implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_mLineFromText.toString())) {
            FunctionRegistry.getInstance().add(new MLineFromText());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_mLineFromText.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                 ...
                 */
                return String.format("ST_MLineFromText(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_mLineFromText.toString()+" function not supported by dialect " + dialect);
    }
