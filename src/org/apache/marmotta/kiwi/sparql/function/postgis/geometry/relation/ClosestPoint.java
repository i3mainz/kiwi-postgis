...
public class ClosestPoint implements NativeFunction {
    ...
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.ST_CLOSESTPOINT.toString())) {
            FunctionRegistry.getInstance().add(new ClosestPoint());
        }
    }
    ...
    public String getURI() {
        return FN_POSTGIS.ST_CLOSESTPOINT.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 2) {
                ...
                return String.format("ST_ClosestPoint(%s,%s)", geom1,geom2);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.ST_CLOSESTPOINT.toString()+" function not supported by dialect " + dialect);
    }
