...
public class LineFromEncodedPolyline extends org.openrdf.query.algebra.evaluation.function.postgis.linestring.constructor.LineFromEncodedPolyline implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_lineFromEncodedPolyline.toString())) {
    ...
    public String getURI() {
        return FN_POSTGIS.st_lineFromEncodedPolyline.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                 ...
                 */
                return String.format("ST_LineFromEncodedPolyline(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_lineFromEncodedPolyline.toString()+" function not supported by dialect " + dialect);
    }
