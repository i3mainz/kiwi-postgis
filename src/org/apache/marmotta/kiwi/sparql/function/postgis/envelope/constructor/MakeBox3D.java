package org.apache.marmotta.kiwi.sparql.function.postgis.envelope.constructor;
...
public class MakeBox3D extends org.openrdf.query.algebra.evaluation.function.postgis.envelope.constructor.MakeBox3D implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_makeBox3D.toString())) {
            FunctionRegistry.getInstance().add(new MakeBox3D());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_makeBox3D.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                 ...
                 */
                return String.format("ST_MakeBox3D(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_makeBox3D.toString()+" function not supported by dialect " + dialect);
    }
