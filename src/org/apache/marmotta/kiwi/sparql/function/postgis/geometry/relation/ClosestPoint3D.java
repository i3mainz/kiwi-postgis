...
public class ClosestPoint3D implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.ST_CLOSESTPOINT3D.toString())) {
            FunctionRegistry.getInstance().add(new ClosestPoint3D());
        }
    }
    ...
    public String getURI() {
        return FN_POSTGIS.ST_CLOSESTPOINT3D.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 2) {
                ...
                return String.format("ST_ClosestPoint3D(%s,%s)", geom1,geom2);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.ST_CLOSESTPOINT3D.toString()+" function not supported by dialect " + dialect);
    }
