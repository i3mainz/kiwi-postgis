...
public class CollectionHomogenize extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.CollectionHomogenize implements NativeFunction {
    ...
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.ST_COLLECTIONHOMOGENIZE.toString())) {
            FunctionRegistry.getInstance().add(new CollectionHomogenize());
        }
    }
    ...
    public String getURI() {
        return FN_POSTGIS.ST_COLLECTIONHOMOGENIZE.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 2) {
                ...
                return String.format("ST_CollectionHomogenize(%s)", geom1);
            }
        }
