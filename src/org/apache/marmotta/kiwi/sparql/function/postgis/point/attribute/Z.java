...
public class Z extends org.openrdf.query.algebra.evaluation.function.postgis.point.attribute.Z implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_z.toString())) {
            FunctionRegistry.getInstance().add(new Z());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_z.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_Z(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_z.toString()+" function not supported by dialect " + dialect);
    }
