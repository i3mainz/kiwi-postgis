...
public class AsTWKB implements NativeFunction {
    ...
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.ST_ASTWKB.toString())) {
            FunctionRegistry.getInstance().add(new AsTWKB());
        }
    }
    ...
    public String getURI() {
        return FN_POSTGIS.ST_ASTWKB.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_AsTWKB(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.ST_ASTWKB.toString()+" function not supported by dialect " + dialect);
    }
