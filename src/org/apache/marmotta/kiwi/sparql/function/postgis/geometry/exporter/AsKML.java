...
public class AsKML extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.exporter.AsKML implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.ST_ASKML.toString())) {
            FunctionRegistry.getInstance().add(new AsKML());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.ST_ASKML.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_KML(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.ST_ASKML.toString()+" function not supported by dialect " + dialect);
    }
