...
public class Length implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_Length.toString())) {
    ...
    public String getURI() {
        return FN_POSTGIS.st_Length.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_Length(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException("Length function not supported by dialect " + dialect);
    }
