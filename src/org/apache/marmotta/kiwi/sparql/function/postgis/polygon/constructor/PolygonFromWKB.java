...
public class PolygonFromWKB extends org.openrdf.query.algebra.evaluation.function.postgis.polygon.constructor.PolygonFromWKB implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_polygonFromWKB.toString())) {
            FunctionRegistry.getInstance().add(new PolygonFromWKB());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_polygonFromWKB.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                 ...
                 */
                return String.format("ST_PolygonFromWKB(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_polygonFromWKB.toString()+" function not supported by dialect " + dialect);
    }
