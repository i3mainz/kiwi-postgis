...
public class AsSVG extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.exporter.AsSVG implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.ST_ASSVG.toString())) {
            FunctionRegistry.getInstance().add(new AsSVG());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.ST_ASSVG.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_AsSVG(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.ST_ASSVG.toString()+" function not supported by dialect " + dialect);
    }
