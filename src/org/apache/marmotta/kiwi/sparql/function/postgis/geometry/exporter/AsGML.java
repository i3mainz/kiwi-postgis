...
public class AsGML implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.ST_ASGML.toString())) {
            FunctionRegistry.getInstance().add(new AsGML());
        }
    }
    ...
    public String getURI() {
        return FN_POSTGIS.ST_ASGML.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_AsGML(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.ST_ASGML.toString()+" function not supported by dialect " + dialect);
    }
