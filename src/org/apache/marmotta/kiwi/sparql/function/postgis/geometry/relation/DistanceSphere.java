...
import org.apache.marmotta.kiwi.sparql.function.postgis.envelope.relation.BBOXRightOf;
...
public class DistanceSphere extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.relation.DistanceSphere implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.ST_DISTANCESPHERE.toString())) {
            FunctionRegistry.getInstance().add(new DistanceSphere());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.ST_DISTANCESPHERE.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 2) {
                ...
                return String.format("ST_DistanceSphere(%s,%s)", geom1,geom2);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.ST_DISTANCESPHERE.toString()+" function not supported by dialect " + dialect);
    }
