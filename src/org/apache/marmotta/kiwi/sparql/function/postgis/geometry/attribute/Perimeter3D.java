...
public class Perimeter3D extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.attribute.Perimeter3D implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_perimeter3D.toString())) {
            FunctionRegistry.getInstance().add(new Perimeter3D());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_perimeter3D.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_Perimeter3D(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_perimeter3D.toString()+" function not supported by dialect " + dialect);
    }
