...
public class WorldToRasterCoordY extends org.openrdf.query.algebra.evaluation.function.postgis.raster.attribute.WorldToRasterCoordY implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_rasterToWorldCoordY.toString())) {
            FunctionRegistry.getInstance().add(new RasterToWorldCoordY());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_rasterToWorldCoordY.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_RasterToWorldCoordY(%s,%s,%s)", geom1,longitude,latitude);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_rasterToWorldCoordY.toString()+" function not supported by dialect " + dialect);
    }
