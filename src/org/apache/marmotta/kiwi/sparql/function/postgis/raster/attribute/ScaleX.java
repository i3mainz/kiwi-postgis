...
public class ScaleX extends org.openrdf.query.algebra.evaluation.function.postgis.raster.attribute.ScaleX implements NativeFunction  {

	 // auto-register for SPARQL environment
   static {
       if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_scaleX.toString())) {
           FunctionRegistry.getInstance().add(new ScaleX());
       }
   }

   @Override
   public String getURI() {
       return FN_POSTGIS.st_scaleX.stringValue();
   }
   ...
   public String getNative(KiWiDialect dialect, String... args) {
       if (dialect instanceof PostgreSQLDialect) {
           if (args.length == 1) {
               ...
               return String.format("ST_ScaleX(%s)", geom1,longitude,latitude);
           }
       }
       throw new UnsupportedOperationException(FN_POSTGIS.st_scaleX.toString()+" function not supported by dialect " + dialect);
   }
