...
public class IsValidReason extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.attribute.IsValidReason implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_isValidReason.toString())) {
            FunctionRegistry.getInstance().add(new IsValidReason());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_isValidReason.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_IsValidReason(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_isValidReason.toString()+" function not supported by dialect " + dialect);
    }
