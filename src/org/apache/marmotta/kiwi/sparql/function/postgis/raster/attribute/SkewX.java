...
public class SkewX extends org.openrdf.query.algebra.evaluation.function.postgis.raster.attribute.SkewX implements NativeFunction  {

	 // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_skewX.toString())) {
            FunctionRegistry.getInstance().add(new SkewX());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_skewX.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_SkewX(%s)", geom1,longitude,latitude);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_skewX.toString()+" function not supported by dialect " + dialect);
    }
