...
public class RasterToWorldCoord extends org.openrdf.query.algebra.evaluation.function.postgis.raster.attribute.RasterToWorldCoord implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_rasterToWorldCoord.toString())) {
            FunctionRegistry.getInstance().add(new RasterToWorldCoord());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_rasterToWorldCoord.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_RasterToWorldCoord(%s,%s,%s)", geom1,longitude,latitude);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_rasterToWorldCoord.toString()+" function not supported by dialect " + dialect);
    }
