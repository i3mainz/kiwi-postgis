...
public class LineToCurve implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_lineToCurve.toString())) {
            FunctionRegistry.getInstance().add(new LineToCurve());
        }
    }
    ...
    public String getURI() {
        return FN_POSTGIS.st_lineToCurve.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_LineToCurve(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_lineToCurve.toString()+" function not supported by dialect " + dialect);
    }
