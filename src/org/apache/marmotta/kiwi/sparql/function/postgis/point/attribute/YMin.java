...
public class YMin extends org.openrdf.query.algebra.evaluation.function.postgis.point.attribute.YMin implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_yMin.toString())) {
            FunctionRegistry.getInstance().add(new YMin());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_yMin.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_YMin(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_yMin.toString()+" function not supported by dialect " + dialect);
    }
