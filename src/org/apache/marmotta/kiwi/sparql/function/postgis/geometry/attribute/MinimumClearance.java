...
public class MinimumClearance extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.attribute.MinimumClearance implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_minimumClearance.toString())) {
            FunctionRegistry.getInstance().add(new MinimumClearance());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_minimumClearance.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_MinimumClearance(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_minimumClearance.toString()+" function not supported by dialect " + dialect);
    }
