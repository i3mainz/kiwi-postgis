...
public class UpperLeftX extends org.openrdf.query.algebra.evaluation.function.postgis.raster.attribute.UpperLeftX implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_upperLeftX.toString())) {
            FunctionRegistry.getInstance().add(new UpperLeftX());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_upperLeftX.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_UpperLeftX(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_upperLeftX.toString()+" function not supported by dialect " + dialect);
    }
