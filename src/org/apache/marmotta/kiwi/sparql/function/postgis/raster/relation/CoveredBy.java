...
public class CoveredBy extends org.openrdf.query.algebra.evaluation.function.postgis.raster.relation.CoveredBy implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_rast_CoveredBy.toString())) {
            FunctionRegistry.getInstance().add(new CoveredBy());
        }
    }
    
    @Override
    public String getURI() {
        return FN_POSTGIS.st_rast_CoveredBy.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 2) {
                ...
                return String.format("ST_CoveredBy(%s,%s)", geom1,geom2);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_rast_CoveredBy.toString()+" function not supported by dialect " + dialect);
    }
