...
import org.apache.marmotta.kiwi.sparql.function.postgis.point.attribute.Zmin;
...
public class MakePoint extends org.openrdf.query.algebra.evaluation.function.postgis.point.constructor.MakePoint implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_makePoint.toString())) {
            FunctionRegistry.getInstance().add(new MakePoint());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_makePoint.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_MakePoint(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_makePoint.toString()+" function not supported by dialect " + dialect);
    }
