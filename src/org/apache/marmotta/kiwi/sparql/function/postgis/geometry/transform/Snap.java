...
public class Snap extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.transform.Snap implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_snap.toString())) {
            FunctionRegistry.getInstance().add(new Snap());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_snap.stringValue();
    }
        ...
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 2) {
                ...
                if (args[1].contains("POINT") || args[1].contains("MULTIPOINT") || args[1].contains("LINESTRING") || args[1].contains("MULTILINESTRING") || args[1].contains("POLYGON") || args[1].contains("MULTIPOLYGON") || args[1].contains("ST_AsText")) {
                    geom2 = String.format("ST_GeomFromText(%s,%s)", args[0], SRID_default);
                }
                return String.format("ST_Snap(%s,%s)", geom1,geom2);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_snap.toString()+" function not supported by dialect " + dialect);
    }
