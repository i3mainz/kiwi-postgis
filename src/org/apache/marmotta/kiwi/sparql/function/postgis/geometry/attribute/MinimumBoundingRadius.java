...
public class MinimumBoundingRadius extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.attribute.MinimumBoundingRadius implements NativeFunction {
    ...
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_minimumBoundingRadius.toString())) {
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_MinimumBoundingRadius(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_minimumBoundingRadius.toString()+" function not supported by dialect " + dialect);
    }
