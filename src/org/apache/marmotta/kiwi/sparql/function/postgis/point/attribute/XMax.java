...
public class XMax extends org.openrdf.query.algebra.evaluation.function.postgis.point.attribute.XMax implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_xMax.toString())) {
    ...
    public String getURI() {
        return FN_POSTGIS.st_xMax.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_XMax(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_xMax.toString()+" function not supported by dialect " + dialect);
    }
