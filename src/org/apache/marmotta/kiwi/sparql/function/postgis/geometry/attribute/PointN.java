...
public class PointN implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_pointN.toString())) {
            FunctionRegistry.getInstance().add(new PointN());
        }
    }
    ...
    public String getURI() {
        return FN_POSTGIS.st_pointN.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 2) {
                String geom1 = args[0];
                Integer pointn = Integer.valueOf(args[1]);
