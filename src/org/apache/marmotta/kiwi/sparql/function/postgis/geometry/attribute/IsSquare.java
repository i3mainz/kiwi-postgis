...
public class IsSquare extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.attribute.IsSquare implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_isSquare.toString())) {
            FunctionRegistry.getInstance().add(new IsSquare());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_isSquare.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_IsSquare(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_isSquare.stringValue()+" function not supported by dialect " + dialect);
    }
