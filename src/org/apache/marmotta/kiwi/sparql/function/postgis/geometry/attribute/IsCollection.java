...
public class IsCollection extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.attribute.IsCollection implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_isCollection.toString())) {
    ...
    public String getURI() {
        return FN_POSTGIS.st_isCollection.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_IsCollection(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException("IsCollection function not supported by dialect " + dialect);
    }
