...
public class NumBands extends org.openrdf.query.algebra.evaluation.function.postgis.raster.attribute.NumBands implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_numBands.toString())) {
            FunctionRegistry.getInstance().add(new NumBands());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_numBands.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_NumBands(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_numBands.toString()+" function not supported by dialect " + dialect);
    }
