...
public class LocateAlong extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.attribute.LocateAlong implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_locateAlong.toString())) {
            FunctionRegistry.getInstance().add(new Length());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_locateAlong.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_LocateAlong(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_locateAlong.stringValue()+" function not supported by dialect " + dialect);
    }
