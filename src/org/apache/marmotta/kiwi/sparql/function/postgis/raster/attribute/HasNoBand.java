...
import org.apache.marmotta.kiwi.sparql.function.postgis.polygon.Orientation;
import org.apache.marmotta.kiwi.vocabulary.FN_POSTGIS;
import org.openrdf.query.algebra.evaluation.function.FunctionRegistry;

public class HasNoBand extends org.openrdf.query.algebra.evaluation.function.postgis.raster.attribute.HasNoBand implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_hasNoBand.toString())) {
            FunctionRegistry.getInstance().add(new HasNoBand());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_hasNoBand.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                String geom1 = args[0];
                Integer noband = Integer.valueOf(args[1]);
                ...
                return String.format("ST_HasNoBand(%s,%s)", geom1,noband);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_hasNoBand.toString()+" function not supported by dialect " + dialect);
    }
