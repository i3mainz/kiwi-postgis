...
public class NumDistinctGeometries extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.attribute.NumDistinctGeometries implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_numDistinctGeometries.toString())) {
            FunctionRegistry.getInstance().add(new NumGeometries());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_numDistinctGeometries.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_NumGeometries(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_numDistinctGeometries.toString()+" function not supported by dialect " + dialect);
    }
