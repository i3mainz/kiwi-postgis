...
public class Zmin implements NativeFunction {
    ...
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_zMin.toString())) {
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_Zmin(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_zMin.toString()+" function not supported by dialect " + dialect);
    }
