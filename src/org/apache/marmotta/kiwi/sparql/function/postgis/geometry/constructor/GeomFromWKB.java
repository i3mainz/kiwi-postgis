...
public class GeomFromWKB extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.constructor.GeomFromWKB implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_geomFromWKB.toString())) {
            FunctionRegistry.getInstance().add(new GeomFromWKB());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_geomFromWKB.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                 ...
                 */
                return String.format("ST_GeomFromWKB(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_geomFromWKB.toString()+" function not supported by dialect " + dialect);
    }
