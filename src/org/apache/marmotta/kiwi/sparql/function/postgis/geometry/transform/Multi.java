...
public class Multi extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.Multi implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_multi.toString())) {
            FunctionRegistry.getInstance().add(new Multi());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_multi.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 2) {
                ...
                return String.format("ST_Multi(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_multi.toString()+" function not supported by dialect " + dialect);
    }
