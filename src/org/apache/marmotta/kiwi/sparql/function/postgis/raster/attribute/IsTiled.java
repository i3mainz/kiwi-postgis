...
public class IsTiled extends org.openrdf.query.algebra.evaluation.function.postgis.raster.attribute.IsTiled implements NativeFunction  {

	 // auto-register for SPARQL environment
static {
    if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_isTiled.toString())) {
        FunctionRegistry.getInstance().add(new IsTiled());
    }
}

@Override
public String getURI() {
    return FN_POSTGIS.st_isTiled.stringValue();
}
...
public String getNative(KiWiDialect dialect, String... args) {
    if (dialect instanceof PostgreSQLDialect) {
        if (args.length == 1) {
            ...
            return String.format("ST_IsTiled(%s)", geom1,longitude,latitude);
        }
    }
    throw new UnsupportedOperationException(FN_POSTGIS.st_isTiled.toString()+" function not supported by dialect " + dialect);
}
