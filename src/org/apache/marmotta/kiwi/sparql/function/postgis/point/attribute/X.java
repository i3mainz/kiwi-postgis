package org.apache.marmotta.kiwi.sparql.function.postgis.point.attribute;

import org.apache.marmotta.kiwi.persistence.KiWiDialect;
import org.apache.marmotta.kiwi.persistence.pgsql.PostgreSQLDialect;
import org.apache.marmotta.kiwi.sparql.builder.ValueType;
import org.apache.marmotta.kiwi.sparql.function.NativeFunction;
import org.apache.marmotta.kiwi.sparql.function.postgis.envelope.relation.BBOXAbove;
...
public class X extends org.openrdf.query.algebra.evaluation.function.postgis.point.attribute.X implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_x.toString())) {
            FunctionRegistry.getInstance().add(new X());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_x.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_X(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_x.toString()+" function not supported by dialect " + dialect);
    }
