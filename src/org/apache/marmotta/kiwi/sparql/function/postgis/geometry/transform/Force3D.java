...
public class Force3D extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.transform.Force3D implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_force3d.toString())) {
    ...
    public String getURI() {
        return FN_POSTGIS.st_force3d.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_Force3D(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_force3d.toString()+" function not supported by dialect " + dialect);
    }
