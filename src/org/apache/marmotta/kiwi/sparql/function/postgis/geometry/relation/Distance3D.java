...
public class Distance3D extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.relation.Distance3D implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.ST_DISTANCE3D.toString())) {
            FunctionRegistry.getInstance().add(new Distance3D());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.ST_DISTANCE3D.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 2) {
                ...
                return String.format("ST_Distance3D(%s,%s)", geom1,geom2);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.ST_DISTANCE3D.toString()+" function not supported by dialect " + dialect);
    }
