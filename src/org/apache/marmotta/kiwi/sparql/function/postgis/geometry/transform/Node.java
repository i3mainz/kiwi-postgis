...
public class Node implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_node.toString())) {
            FunctionRegistry.getInstance().add(new Node());
        }
    }
    ...
    public String getURI() {
        return FN_POSTGIS.st_node.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 2) {
                ...
                return String.format("ST_Node(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_node.toString()+" function not supported by dialect " + dialect);
    }
