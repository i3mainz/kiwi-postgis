...
public class Dimension implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.ST_DIMENSION.toString())) {
            FunctionRegistry.getInstance().add(new Dimension());
        }
    }
    ...
    public String getURI() {
        return FN_POSTGIS.ST_DIMENSION.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_Dimension(%s)", geom1);
            }
        }
