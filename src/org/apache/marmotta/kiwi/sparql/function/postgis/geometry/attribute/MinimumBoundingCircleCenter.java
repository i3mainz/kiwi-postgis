...
import org.apache.marmotta.kiwi.sparql.function.postgis.geometry.constructor.GeomFromGeoJSON;
import org.apache.marmotta.kiwi.vocabulary.FN_POSTGIS;
import org.openrdf.query.algebra.evaluation.function.FunctionRegistry;

public class MinimumBoundingCircleCenter extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.attribute.MinimumBoundingCircleCenter implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_minimumBoundingCircleCenter.toString())) {
            FunctionRegistry.getInstance().add(new MinimumBoundingCircleCenter());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_minimumBoundingCircleCenter.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                 ...
                 */
                return String.format("ST_MinimumBoundingCircleCenter(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_minimumBoundingCircleCenter.toString()+" function not supported by dialect " + dialect);
    }
