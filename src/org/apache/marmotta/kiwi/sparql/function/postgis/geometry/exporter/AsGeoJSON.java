package org.apache.marmotta.kiwi.sparql.function.postgis.geometry.exporter;

import org.apache.marmotta.kiwi.persistence.KiWiDialect;
import org.apache.marmotta.kiwi.persistence.pgsql.PostgreSQLDialect;
import org.apache.marmotta.kiwi.sparql.builder.ValueType;
import org.apache.marmotta.kiwi.sparql.function.NativeFunction;
import org.apache.marmotta.kiwi.sparql.function.postgis.geometry.attribute.NumGeometries;
...
public class AsGeoJSON extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.exporter.AsGeoJSON implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.ST_ASGEOJSON.toString())) {
            FunctionRegistry.getInstance().add(new AsGeoJSON());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.ST_ASGEOJSON.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_AsGeoJSON(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.ST_ASGEOJSON.toString()+" function not supported by dialect " + dialect);
    }
