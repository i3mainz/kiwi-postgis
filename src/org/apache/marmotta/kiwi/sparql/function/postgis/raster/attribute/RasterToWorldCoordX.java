...
public class RasterToWorldCoordX extends org.openrdf.query.algebra.evaluation.function.postgis.raster.attribute.RasterToWorldCoordX implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_rasterToWorldCoordX.toString())) {
            FunctionRegistry.getInstance().add(new RasterToWorldCoordX());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_rasterToWorldCoordX.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_RasterToWorldCoordX(%s,%s,%s)", geom1,longitude,latitude);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_rasterToWorldCoordX.toString()+" function not supported by dialect " + dialect);
    }
