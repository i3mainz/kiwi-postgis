...
public class SwapOrdinates extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.transform.SwapOrdinates implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_swapOrdinates.toString())) {
            FunctionRegistry.getInstance().add(new SwapOrdinates());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_swapOrdinates.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_SwapOrdinates(%s,%s)", geom1,ordinates);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_swapOrdinates.toString()+" function not supported by dialect " + dialect);
    }
