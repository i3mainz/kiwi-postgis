...
public class MakeBox2D extends org.openrdf.query.algebra.evaluation.function.postgis.envelope.constructor.MakeBox2D implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_makeBox2D.toString())) {
            FunctionRegistry.getInstance().add(new MakeBox2D());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_makeBox2D.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                 ...
                 */
                return String.format("ST_MakeBox2D(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_makeBox2D.toString()+" function not supported by dialect " + dialect);
    }
