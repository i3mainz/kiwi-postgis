package org.apache.marmotta.kiwi.sparql.function.postgis.polygon.constructor;
...
public class PolygonFromText extends org.openrdf.query.algebra.evaluation.function.postgis.polygon.constructor.PolygonFromText implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_polygonFromText.toString())) {
            FunctionRegistry.getInstance().add(new PolygonFromText());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_polygonFromText.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                 ...
                 */
                return String.format("ST_PolygonFromText(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_polygonFromText.toString()+" function not supported by dialect " + dialect);
    }
