...
public class IsValidDetail extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.attribute.IsValidDetail implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_isValidDetail.toString())) {
            FunctionRegistry.getInstance().add(new IsValid());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_isValidDetail.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_IsValid(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_isValidDetail.toString()+" function not supported by dialect " + dialect);
    }
