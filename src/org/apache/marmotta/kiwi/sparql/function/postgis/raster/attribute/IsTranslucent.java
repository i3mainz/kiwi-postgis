...
public class IsTranslucent extends org.openrdf.query.algebra.evaluation.function.postgis.raster.attribute.IsTranslucent implements NativeFunction  {

	 // auto-register for SPARQL environment
static {
    if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_isTranslucent.toString())) {
        FunctionRegistry.getInstance().add(new IsTranslucent());
    }
}

@Override
public String getURI() {
    return FN_POSTGIS.st_isTranslucent.stringValue();
}
...
public String getNative(KiWiDialect dialect, String... args) {
    if (dialect instanceof PostgreSQLDialect) {
        if (args.length == 1) {
            ...
            return String.format("ST_IsTranslucent(%s)", geom1,longitude,latitude);
        }
    }
    throw new UnsupportedOperationException(FN_POSTGIS.st_isTranslucent.toString()+" function not supported by dialect " + dialect);
}
