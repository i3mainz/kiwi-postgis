...
public class UpperLeftY extends org.openrdf.query.algebra.evaluation.function.postgis.raster.attribute.UpperLeftY implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_upperLeftY.toString())) {
            FunctionRegistry.getInstance().add(new UpperLeftY());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_upperLeftY.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_UpperLeftY(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_upperLeftY.toString()+" function not supported by dialect " + dialect);
    }
