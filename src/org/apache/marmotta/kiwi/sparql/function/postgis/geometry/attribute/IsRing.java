...
public class IsRing extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.attribute.IsRing implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_isRing.toString())) {
            FunctionRegistry.getInstance().add(new IsRing());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_isRing.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_IsRing(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_isRing.toString()+" function not supported by dialect " + dialect);
    }
