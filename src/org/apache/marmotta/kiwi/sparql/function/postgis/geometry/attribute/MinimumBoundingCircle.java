...
public class MinimumBoundingCircle extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.attribute.MinimumBoundingCircle implements NativeFunction {
    ...
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_minimumBoundingCircle.toString())) {
    ...
    public String getURI() {
        return FN_POSTGIS.st_minimumBoundingCircle.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_MinimumBoundingCircle(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_minimumBoundingCircle.toString()+" function not supported by dialect " + dialect);
    }
