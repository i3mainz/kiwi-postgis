...
public class AsGeoRSS extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.exporter.AsGeoRSS implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.ST_ASGEORSS.toString())) {
            FunctionRegistry.getInstance().add(new AsGeoRSS());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.ST_ASGEORSS.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_AsGeoRSS(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.ST_ASGEORSS.toString()+" function not supported by dialect " + dialect);
    }
