...
public class StartPoint extends org.openrdf.query.algebra.evaluation.function.postgis.linestring.attribute.StartPoint implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_startPoint.toString())) {
    ...
    public String getURI() {
        return FN_POSTGIS.st_startPoint.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_StartPoint(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_startPoint.toString()+" function not supported by dialect " + dialect);
    }
