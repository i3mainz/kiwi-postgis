...
public class Dump extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.attribute.Dump implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_dump.toString())) {
            FunctionRegistry.getInstance().add(new Dump());
        }
    }
    ...
    public String getURI() {
        return FN_POSTGIS.st_dump.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 2) {
                ...
                return String.format("ST_Dump(%s)", geom1,geomn);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_dump.toString()+" function not supported by dialect " + dialect);
    }
