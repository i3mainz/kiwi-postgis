...
public class IsSimple extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.attribute.IsSimple implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_isSimple.toString())) {
            FunctionRegistry.getInstance().add(new IsSimple());
        }
    }
    ...
    public String getURI() {
        return FN_POSTGIS.st_isSimple.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_IsSimple(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_isSimple.toString()+" function not supported by dialect " + dialect);
    }
