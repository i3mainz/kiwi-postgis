...
public class SkewY extends org.openrdf.query.algebra.evaluation.function.postgis.raster.attribute.SkewY implements NativeFunction  {

	 // auto-register for SPARQL environment
   static {
       if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_skewY.toString())) {
           FunctionRegistry.getInstance().add(new SkewY());
       }
   }

   @Override
   public String getURI() {
       return FN_POSTGIS.st_skewY.stringValue();
   }
   ...
   public String getNative(KiWiDialect dialect, String... args) {
       if (dialect instanceof PostgreSQLDialect) {
           if (args.length == 1) {
               ...
               return String.format("ST_SkewY(%s)", geom1,longitude,latitude);
           }
       }
       throw new UnsupportedOperationException(FN_POSTGIS.st_skewY.toString()+" function not supported by dialect " + dialect);
   }
