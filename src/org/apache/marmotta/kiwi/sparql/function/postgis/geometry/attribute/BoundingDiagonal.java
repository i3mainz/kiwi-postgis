...
public class BoundingDiagonal implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.ST_BOUNDINGDIAGONAL.toString())) {
            FunctionRegistry.getInstance().add(new BoundingDiagonal());
        }
    }
    ...
    public String getURI() {
        return FN_POSTGIS.ST_BOUNDINGDIAGONAL.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_BoundingDiagonal(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.ST_BOUNDINGDIAGONAL.toString()+" function not supported by dialect " + dialect);
    }
