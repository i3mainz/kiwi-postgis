...
public class Within extends org.openrdf.query.algebra.evaluation.function.postgis.raster.relation.Within implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_rast_Within.toString())) {
            FunctionRegistry.getInstance().add(new Within());
        }
    }
    
    @Override
    public String getURI() {
        return FN_POSTGIS.st_rast_Within.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        ...
        throw new UnsupportedOperationException(FN_POSTGIS.st_rast_Within.toString()+" function not supported by dialect " + dialect);
    }
