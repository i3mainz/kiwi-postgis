...
public class GeometryType implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_geometryType.toString())) {
            FunctionRegistry.getInstance().add(new GeometryType());
        }
    }
    ...
    public String getURI() {
        return FN_POSTGIS.st_geometryType.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_GeometryType(%s)", geom1);
            }
        }
