...
public class GeometricMedian extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.attribute.GeometricMedian implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_geometricMedian.toString())) {
            FunctionRegistry.getInstance().add(new GeometryN());
        }
    }
    ...
    public String getURI() {
        return FN_POSTGIS.st_geometricMedian.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 2) {
                String geom1 = args[0];
                Integer geomn = Integer.valueOf(args[1]);
                ...
                return String.format("ST_GeometricMedian(%s,%s)", geom1,geomn);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_geometricMedian.toString()+" function not supported by dialect " + dialect);
    }
