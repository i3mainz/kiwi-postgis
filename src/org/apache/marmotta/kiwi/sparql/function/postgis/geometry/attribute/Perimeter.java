...
public class Perimeter implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_perimeter.toString())) {
            FunctionRegistry.getInstance().add(new Perimeter());
        }
    }
    ...
    public String getURI() {
        return FN_POSTGIS.st_perimeter.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_Perimeter(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_perimeter.toString()+" function not supported by dialect " + dialect);
    }
