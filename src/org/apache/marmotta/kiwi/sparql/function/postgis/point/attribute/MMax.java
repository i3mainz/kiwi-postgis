...
public class MMax extends org.openrdf.query.algebra.evaluation.function.postgis.point.attribute.MMax implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_mMax.toString())) {
            FunctionRegistry.getInstance().add(new MMax());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_mMax.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_MMax(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_mMax.toString()+" function not supported by dialect " + dialect);
    }
