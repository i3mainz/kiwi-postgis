...
public class IsValidTrajectory extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.attribute.IsValidTrajectory implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_isValidTrajectory.toString())) {
            FunctionRegistry.getInstance().add(new IsValidTrajectory());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_isValidTrajectory.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_IsValidTrajectory(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_isValidTrajectory.toString()+" function not supported by dialect " + dialect);
    }
