...
public class Orientation extends org.openrdf.query.algebra.evaluation.function.postgis.polygon.Orientation implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_orientation.toString())) {
            FunctionRegistry.getInstance().add(new Orientation());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_orientation.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_Orientation(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_orientation.toString()+" function not supported by dialect " + dialect);
    }
