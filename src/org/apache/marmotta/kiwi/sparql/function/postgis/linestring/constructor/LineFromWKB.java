...
public class LineFromWKB extends org.openrdf.query.algebra.evaluation.function.postgis.linestring.constructor.LineFromWKB implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_lineFromWKB.toString())) {
    ...
    public String getURI() {
        return FN_POSTGIS.st_lineFromWKB.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                 ...
                 */
                return String.format("ST_LineFromWKB(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_lineFromWKB.toString()+" function not supported by dialect " + dialect);
    }
