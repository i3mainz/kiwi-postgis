package org.apache.marmotta.kiwi.sparql.function.postgis.geometry.transform;
...
public class RemoveRepeatedPoints extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.editor.RemoveRepeatedPoints implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_removeRepeatedPoints.toString())) {
            FunctionRegistry.getInstance().add(new RemoveRepeatedPoints());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_removeRepeatedPoints.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_RemoveRepeatedPoints(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_removeRepeatedPoints.toString()+" function not supported by dialect " + dialect);
    }
