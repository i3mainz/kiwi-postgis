...
public class NumYTiles extends org.openrdf.query.algebra.evaluation.function.postgis.raster.attribute.NumXTiles implements NativeFunction  {

	 // auto-register for SPARQL environment
  static {
      if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_numYTiles.toString())) {
          FunctionRegistry.getInstance().add(new NumYTiles());
      }
  }

  @Override
  public String getURI() {
      return FN_POSTGIS.st_numYTiles.stringValue();
  }
  ...
  public String getNative(KiWiDialect dialect, String... args) {
      if (dialect instanceof PostgreSQLDialect) {
          if (args.length == 1) {
              ...
              return String.format("ST_NumYTiles(%s)", geom1,longitude,latitude);
          }
      }
      throw new UnsupportedOperationException(FN_POSTGIS.st_numYTiles.toString()+" function not supported by dialect " + dialect);
  }
