...
public class WorldToRasterCoord extends org.openrdf.query.algebra.evaluation.function.postgis.raster.attribute.WorldToRasterCoord implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_worldToRasterCoord.toString())) {
            FunctionRegistry.getInstance().add(new WorldToRasterCoord());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_worldToRasterCoord.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_WorldToRasterCoord(%s,%s,%s)", geom1,longitude,latitude);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_worldToRasterCoord.toString()+" function not supported by dialect " + dialect);
    }
