...
public class NumPoints extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.attribute.NumPoints implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_numPoints.toString())) {
            FunctionRegistry.getInstance().add(new NumPoints());
        }
    }
    
    @Override
    public String getURI() {
        return FN_POSTGIS.st_numPoints.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_NumPoints(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_numPoints.toString()+" function not supported by dialect " + dialect);
    }
