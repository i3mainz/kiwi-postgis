...
public class IsRectangular implements NativeFunction {

	// auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_isRectangle.toString())) {
            FunctionRegistry.getInstance().add(new IsRectangular());
        }
    }
    ...
    public String getURI() {
        return FN_POSTGIS.st_isRectangle.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_IsRectangle(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException("IsRectangular function not supported by dialect " + dialect);
    }
