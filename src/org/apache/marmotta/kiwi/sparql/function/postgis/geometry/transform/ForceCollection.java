...
public class ForceCollection extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.transform.ForceCollection implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_forceCollection.toString())) {
            FunctionRegistry.getInstance().add(new ForceCollection());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_forceCollection.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_ForceCollection(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_forceCollection.toString()+" function not supported by dialect " + dialect);
    }
