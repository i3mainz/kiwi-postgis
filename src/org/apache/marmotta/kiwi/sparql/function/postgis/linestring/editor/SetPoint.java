...
public class SetPoint extends org.openrdf.query.algebra.evaluation.function.postgis.linestring.editor.SetPoint implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_setPoint.toString())) {
            FunctionRegistry.getInstance().add(new SetPoint());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_setPoint.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_SetPoint(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_setPoint.toString()+" function not supported by dialect " + dialect);
    }
