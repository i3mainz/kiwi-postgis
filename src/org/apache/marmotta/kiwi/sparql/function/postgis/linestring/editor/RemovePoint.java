...
public class RemovePoint extends org.openrdf.query.algebra.evaluation.function.postgis.linestring.editor.RemovePoint implements NativeFunction {
    ...
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_removePoint.toString())) {
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_RemovePoint(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_removePoint.toString()+" function not supported by dialect " + dialect);
    }
