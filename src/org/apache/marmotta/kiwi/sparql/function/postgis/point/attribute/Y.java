...
public class Y extends org.openrdf.query.algebra.evaluation.function.postgis.point.attribute.Y implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_y.toString())) {
            FunctionRegistry.getInstance().add(new Y());
        }
    }
    
    @Override
    public String getURI() {
        return FN_POSTGIS.st_y.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_Y(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_y.toString()+" function not supported by dialect " + dialect);
    }
