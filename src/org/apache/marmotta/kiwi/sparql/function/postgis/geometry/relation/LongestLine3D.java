...
public class LongestLine3D implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.ST_LONGESTLINE3D.toString())) {
            FunctionRegistry.getInstance().add(new LongestLine3D());
        }
    }
    ...
    public String getURI() {
        return FN_POSTGIS.ST_LONGESTLINE3D.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 2) {
                ...
                return String.format("ST_LongestLine3D(%s,%s)", geom1,geom2);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.ST_LONGESTLINE3D.toString()+" function not supported by dialect " + dialect);
    }
