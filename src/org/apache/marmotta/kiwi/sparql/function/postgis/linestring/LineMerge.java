package org.apache.marmotta.kiwi.sparql.function.postgis.linestring;
...
public class LineMerge extends org.openrdf.query.algebra.evaluation.function.postgis.linestring.LineMerge implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_lineMerge.toString())) {
            FunctionRegistry.getInstance().add(new LineMerge());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_lineMerge.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_LineMerge(%s,%s)", geom1,ordinates);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_lineMerge.toString()+" function not supported by dialect " + dialect);
    }
