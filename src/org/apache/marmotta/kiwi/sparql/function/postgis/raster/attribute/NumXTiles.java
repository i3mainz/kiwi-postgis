...
public class NumXTiles extends org.openrdf.query.algebra.evaluation.function.postgis.raster.attribute.NumXTiles implements NativeFunction  {

	 // auto-register for SPARQL environment
   static {
       if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_numXTiles.toString())) {
           FunctionRegistry.getInstance().add(new NumXTiles());
       }
   }

   @Override
   public String getURI() {
       return FN_POSTGIS.st_numXTiles.stringValue();
   }
   ...
   public String getNative(KiWiDialect dialect, String... args) {
       if (dialect instanceof PostgreSQLDialect) {
           if (args.length == 1) {
               ...
               return String.format("ST_NumXTiles(%s)", geom1,longitude,latitude);
           }
       }
       throw new UnsupportedOperationException(FN_POSTGIS.st_numXTiles.toString()+" function not supported by dialect " + dialect);
   }
