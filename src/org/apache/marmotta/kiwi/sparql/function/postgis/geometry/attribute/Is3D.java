...
public class Is3D extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.attribute.Is3D implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_is3D.toString())) {
            FunctionRegistry.getInstance().add(new Is3D());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_is3D.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_Is3D(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException("Is3D function not supported by dialect " + dialect);
    }
