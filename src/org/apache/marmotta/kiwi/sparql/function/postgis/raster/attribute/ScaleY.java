...
public class ScaleY extends org.openrdf.query.algebra.evaluation.function.postgis.raster.attribute.ScaleY implements NativeFunction  {

	 // auto-register for SPARQL environment
   static {
       if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_scaleY.toString())) {
           FunctionRegistry.getInstance().add(new ScaleY());
       }
   }

   @Override
   public String getURI() {
       return FN_POSTGIS.st_scaleY.stringValue();
   }
   ...
   public String getNative(KiWiDialect dialect, String... args) {
       if (dialect instanceof PostgreSQLDialect) {
           if (args.length == 1) {
               ...
               return String.format("ST_ScaleY(%s)", geom1,longitude,latitude);
           }
       }
       throw new UnsupportedOperationException(FN_POSTGIS.st_scaleY.toString()+" function not supported by dialect " + dialect);
   }
