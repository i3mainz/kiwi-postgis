...
public class SharedPaths extends org.openrdf.query.algebra.evaluation.function.postgis.linestring.SharedPaths implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_sharedPaths.toString())) {
            FunctionRegistry.getInstance().add(new SharedPaths());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_sharedPaths.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 2) {
                ...
                return String.format("ST_SharedPaths(%s,%s)", geom1,geom2);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_sharedPaths.toString()+" function not supported by dialect " + dialect);
    }
