...
public class LocateBetweenMeasures extends org.openrdf.query.algebra.evaluation.function.postgis.geometry.attribute.LocateBetweenMeasures implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_locateBetweenMeasures.toString())) {
            FunctionRegistry.getInstance().add(new LocateBetweenMeasures());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_locateBetweenMeasures.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 3) {
                ...
                return String.format("ST_LocateBetweenMeasures(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_locateBetweenMeasures.stringValue()+" function not supported by dialect " + dialect);
    }
