...
public class Width extends org.openrdf.query.algebra.evaluation.function.postgis.raster.attribute.Width  implements NativeFunction {

    // auto-register for SPARQL environment
    static {
        if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_width.toString())) {
            FunctionRegistry.getInstance().add(new Width());
        }
    }

    @Override
    public String getURI() {
        return FN_POSTGIS.st_width.stringValue();
    }
    ...
    public String getNative(KiWiDialect dialect, String... args) {
        if (dialect instanceof PostgreSQLDialect) {
            if (args.length == 1) {
                ...
                return String.format("ST_Width(%s)", geom1);
            }
        }
        throw new UnsupportedOperationException(FN_POSTGIS.st_width.toString()+" function not supported by dialect " + dialect);
    }
