...
public class IsGrayscale extends org.openrdf.query.algebra.evaluation.function.postgis.raster.attribute.IsGrayscale implements NativeFunction  {

	 // auto-register for SPARQL environment
static {
    if (!FunctionRegistry.getInstance().has(FN_POSTGIS.st_isGrayscale.toString())) {
        FunctionRegistry.getInstance().add(new IsGrayscale());
    }
}

@Override
public String getURI() {
    return FN_POSTGIS.st_isGrayscale.stringValue();
}
...
public String getNative(KiWiDialect dialect, String... args) {
    if (dialect instanceof PostgreSQLDialect) {
        if (args.length == 1) {
            ...
            return String.format("ST_IsGrayscale(%s)", geom1,longitude,latitude);
        }
    }
    throw new UnsupportedOperationException(FN_POSTGIS.st_isGrayscale.toString()+" function not supported by dialect " + dialect);
}
